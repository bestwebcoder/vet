import { CalendarDays, PawPrint, Receipt, Search, Users, Wallet } from "lucide-react";
  ...
  return (
    <div className="grid gap-6">
      ...
      <SearchField
        ...
        defaultValue={term}
        placeholder="Client name, phone, pet, invoice, reference or date (yyyy-mm-dd)"
        label="Search"
      />
          ...
          {results!.payments.length > 0 ? (
            <ResultCard icon={Wallet} title="Payments">
              {results!.payments.map((payment) => (
                <Link
                  key={payment.id}
                  href={`/doctor/invoices/${payment.invoiceId}`}
                  className="hover:bg-muted/50 -mx-2 flex min-h-11 items-center rounded-lg px-2 py-2 text-sm"
                >
                  <span className="font-medium" data-numeric>
                    {payment.amount}
